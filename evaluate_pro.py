...
def main():
    ...
    eval_pro_data_record = {"epoch":[], "target_returns":[]}
    ...
    df.to_csv(f"{args.log_dir}/eval/tcr-{config.eval_target_cost_returns_list}--trr-{config.eval_target_reward_returns_list}--guidance_w-{config.condition_guidance_w}--sampleT-{config.algo_cfg.sample_temperature}-zero-test.csv", index=False)
