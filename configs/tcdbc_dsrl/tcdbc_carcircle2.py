...
def get_config():
    ...
    config.log_dir_format = "{exp_name}/{env}/{architecture}-gw_{condition_guidance_w}-cdp_{condition_dropout}-{normalizer}-normret_{normalize_returns}/{seed}/2024-4-3-1"
    ...
    config.env = "OfflineCarCircle2Gymnasium-v0"
    config.dataset = "dsrl"
    ...
    config.target_returns = "20.0,20,21.0,40,22.0,80"
    ...
    # data aug configs
    config.aug_deg = 1
    config.aug_max_rew_decrease = 10
    config.aug_max_reward = 30.0
    ...
    config.n_train_step_per_epoch = 10000
