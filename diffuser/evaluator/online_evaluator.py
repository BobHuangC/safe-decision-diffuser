...
class OnlineEvaluator(BaseEvaluator):
    ...
    def __init__(self, config, policy, eval_sampler):
        ...
        self._recent_costs = {method: deque(maxlen=10) for method in self._act_methods}
    ...
    def evaluate(self, epoch: int) -> Dict[str, Any]:
        ...
        for method in self._act_methods:
            ...
            metrics["average_return" + post] = cur_return = np.mean(
                [np.sum(t["rewards"]) for t in trajs]
            )
            self._recent_returns[method].append(cur_return)
            metrics["average_10_return" + post] = np.mean(self._recent_returns[method])
            metrics["best_return" + post] = self._best_returns[method] = max(
                self._best_returns[method], cur_return
            )

            metrics["average_cost" + post] = cur_cost = np.mean(
                [np.sum(t["costs"]) for t in trajs]
            )
            self._recent_costs[method].append(cur_cost)
            metrics["average_10_cost" + post] = np.mean(self._recent_costs[method])
